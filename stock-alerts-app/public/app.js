    ...
    .map((stock) => {
      ...
      return `
        <div class="quote-card">
          ...
          <div class="price-row">
            <span class="price">${q.price?.toFixed ? q.price.toFixed(2) : q.price} ${q.currency || ''}</span>
            <span class="change ${dir}">${sign}${q.changePercent.toFixed(1)}%</span>
          </div>
        </div>`;
    })
    .join('');
...
async function loadStatus() {
  ...
  document.getElementById('push-status').textContent = status.pushConfigured
    ? `Notifications configurees cote serveur — ${status.subscriptionsCount} appareil(s) abonne(s).`
    : 'Notifications non configurees : genere des clefs VAPID (npm run generate-vapid) et renseigne le fichier .env, puis redemarre le serveur.';
}
...
async function enablePush() {
  ...
  if (!publicKey) {
    alert("Cle VAPID absente cote serveur. Lance 'npm run generate-vapid', renseigne le .env, puis redemarre le serveur.");
    return;
  }
