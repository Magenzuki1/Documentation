...
let TOTAL_NORMAL = NORMAL_BANANAS.length; // 150 (+ celles créées par un admin, voir mergeAdminBananas)
const TOTAL_SECRET = SECRET_BANANAS.length; // 9

/* ------------------------------------------------------------
   Bananes créées par un admin (voir le panneau admin) : du contenu
   ajouté EN PLUS de BANANA_DEFS ci-dessus, jamais à sa place. Toujours
   un id >= ADMIN_BANANA_ID_START pour ne jamais entrer en collision
   avec un id "en dur" ajouté plus tard par le développeur. Jamais de
   rareté secrète (sous-système à part, réservé au contenu conçu par
   le développeur).

   Mises en cache dans localStorage et fusionnées ICI, de façon
   SYNCHRONE, avant que app.js charge la sauvegarde du joueur :
   sanitizeState() purge tout id de banane inconnu de BANANAS_BY_ID,
   donc sans cette fusion synchrone en amont, une banane admin déjà
   découverte disparaîtrait de la collection à chaque rechargement de
   page, le temps que CLOUD.init() la resynchronise en asynchrone. */
const ADMIN_BANANA_ID_START = 100000;
const ADMIN_BANANAS_CACHE_KEY = "banana-collector-admin-bananas-v1";
const ADMIN_BANANA_EMOJI_WHITELIST = [
  "🍌", "🍎", "🍇", "🍓", "🍒", "🍑", "🍍", "🥝", "🥥", "🍉",
  "🍊", "🍋", "🥑", "🍅", "🌽", "🥕", "👻", "⭐", "🌙", "🔥",
  "💎", "🎩", "👑", "🦄",
];

function mergeAdminBananas(rows) {
  for (const row of rows || []) {
    if (!row || !Number.isInteger(row.id) || row.id < ADMIN_BANANA_ID_START) continue;
    if (!RARITY_ORDER.includes(row.rarity) || row.rarity === "secrete") continue;
    if (!ADMIN_BANANA_EMOJI_WHITELIST.includes(row.emoji)) continue;
    const hue = Number.isInteger(row.hue) ? Math.max(0, Math.min(359, row.hue)) : 0;
    const banana = {
      id: row.id,
      name: typeof row.name === "string" && row.name.trim() ? row.name.trim() : "Banane mystère",
      rarity: row.rarity,
      emoji: row.emoji,
      image: null,
      deco: hue ? { filter: `hue-rotate(${hue}deg)` } : null,
      secret: false,
      value: Number.isFinite(row.value) && row.value > 0 ? Math.round(row.value) : 10,
      active: row.active !== false,
    };
    const existingIndex = BANANAS.findIndex((b) => b.id === banana.id);
    if (existingIndex >= 0) {
      BANANAS[existingIndex] = banana;
    } else {
      BANANAS.push(banana);
    }
    BANANAS_BY_ID[banana.id] = banana;
  }
  // NORMAL_BANANAS est dérivé de BANANAS (aucune banane admin n'est jamais
  // secrète) : le plus simple et le plus sûr est de le reconstruire
  // entièrement plutôt que de risquer un doublon lors d'une fusion répétée.
  NORMAL_BANANAS.length = 0;
  NORMAL_BANANAS.push(...BANANAS.filter((b) => !b.secret));
  TOTAL_NORMAL = NORMAL_BANANAS.length;
}

function loadAdminBananasCache() {
  try {
    const raw = localStorage.getItem(ADMIN_BANANAS_CACHE_KEY);
    if (raw) mergeAdminBananas(JSON.parse(raw));
  } catch (e) {
    // Cache corrompu/absent : on retombe sur les bananes statiques en
    // attendant la prochaine synchronisation en ligne.
  }
}
loadAdminBananasCache();

function setAdminBananasCache(rows) {
  mergeAdminBananas(rows);
  try {
    localStorage.setItem(ADMIN_BANANAS_CACHE_KEY, JSON.stringify(rows));
  } catch (e) {
